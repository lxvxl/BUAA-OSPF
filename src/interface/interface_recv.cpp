...
void handle_recv_dd(OSPFDD *dd_packet, Interface *interface) {
    ...
    switch (neighbor->state) {
        ...
        case FULL:
            ...
            if (dd_packet->b_I) {
                std::cout<<"got a dd packet with b_I at FULL of LOADING state\n";
                neighbor->event_seq_num_mismatch();
                handle_recv_dd(dd_packet, interface);
                break;
            }
            if (dd_packet->b_I == last_dd->b_I
                    && dd_packet->b_M == last_dd->b_M
                ...
                std::cout<<"got a wrong DD packet!\n";
                dd_packet->show();
                neighbor->event_seq_num_mismatch();
            }
        default:
...
void handle_recv_lsu(OSPFLSU *lsu_packet, Interface *interface, uint32_t saddr, uint32_t daddr) {
    ...
    for (LSAHeader* v_lsa : received_v_lsas) {
        ...
        //如果这个包时间达到最大，且数据库中没有实例，且没有邻居处在EXCHAGE或LOADING状态
        if (r_lsa == NULL && v_lsa->ls_age == router::config::max_age) {
            bool flag = false;
        ...
        if (r_lsa == NULL || v_lsa->ls_seq_num > r_lsa->ls_seq_num) {
            //如果该实例收到保护
            ...
            //    continue;
            //}
            //if the lsa was generated by this router
            LSADatabase &db = router::lsa_db;
            if (v_lsa->advertising_router == router::router_id) {
                std::cout<<"got a self-generated lsa! we got:\n";
                v_lsa->show();
                db.seq_num = v_lsa->ls_seq_num + 1;
                if (v_lsa->ls_type == ROUTER) {
                    db.generate_router_lsa();
                } else if (v_lsa->ls_type == NETWORK) {
                    if (interface->dr == interface->ip) {
                        db.generate_network_lsa(interface);
                    } else {
                        v_lsa->ls_age = router::config::max_age;
                        std::cout<<"we flood:\n"<<std::endl;
                        ((NetworkLSA*)v_lsa)->show();
                        LSADatabase::flood(v_lsa, NULL, DROTHER);
                    }
                }
                continue;
            }
